...
/**
 * Build TikTok live stream URL from username
 * @param {string} username - TikTok username/uniqueId
 * @returns {string} Full TikTok live URL
 */
 ...
 *
 * NOTE: TikTok is excluded here because it uses a separate Playwright client
 * with session authentication (see frontend/app/api/chat/start/route.js)
 */
function buildUrlsFromPlatforms(platforms) {
  ...
  // TikTok is NOT included here - it uses a separate Playwright client with session auth
  // See: frontend/app/api/chat/start/route.js (HYBRID MODE)
