...
class TTSServerManager {
   ...
   * Priority: venv Python > system Python
   */
  findPythonExecutable(ttsEngine) {
    const pythonPaths = [
      // 1. Venv Python (highest priority)
      path.join(this.getBasePath(), 'kokoro-tts', '.venv', 'Scripts', 'python.exe'),  // Windows
      path.join(this.getBasePath(), 'kokoro-tts', '.venv', 'bin', 'python'),          // Linux/Mac
    ...
    console.log(`[TTS Manager] Using system Python for ${ttsEngine}`);
    return 'python';
  }
