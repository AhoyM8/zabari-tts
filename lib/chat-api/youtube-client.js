...
class YouTubeChatClient {
  constructor(options) {
    ...
    this.pollingDelay = options.pollingDelay || 5000; // 5 seconds default to avoid quota limits
  ...
  /**
   * Get channel's current live stream video ID from username or handle
   * @param {string} channelIdentifier - Channel username, handle (@username), or channel ID
   * @param {string} apiKey - YouTube API key
   * @returns {string} Video ID of current live stream
   */
  static async getLiveVideoIdFromChannel(channelIdentifier, apiKey) {
    try {
      // Remove @ if present for handle
      const cleanIdentifier = channelIdentifier.replace(/^@/, '');

      // First, try to find the channel by username/handle
      let channelId = null;

      // Try search for channel by name/handle
      const searchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=channel&q=${encodeURIComponent(cleanIdentifier)}&maxResults=1&key=${apiKey}`;
      ...
      if (!channelId) {
        ...
        channelId = cleanIdentifier;
      }

      // Now search for live streams on this channel
      const liveSearchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelId}&eventType=live&type=video&maxResults=1&key=${apiKey}`;
      const liveResponse = await fetch(liveSearchUrl);

      if (!liveResponse.ok) {
        const errorData = await liveResponse.json();
        throw new Error(`YouTube API error: ${errorData.error?.message || liveResponse.statusText}`);
      }

      const liveData = await liveResponse.json();

      if (!liveData.items || liveData.items.length === 0) {
        throw new Error(`No live stream found for channel: ${channelIdentifier}`);
      }

      return liveData.items[0].id.videoId;

    } catch (error) {
      throw new Error(`Failed to get live video from channel "${channelIdentifier}": ${error.message}`);
    }
  }
