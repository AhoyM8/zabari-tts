...
const TikTokChatClient = require('./tiktok-client');
...
async function initializeChatClients(config) {
  ...
  if (platforms.tiktok?.enabled) {
    ...
    if (username) {
      const tiktokClient = new TikTokChatClient({
        channelName: username, // TikTok uses username/uniqueId
        onMessage: handleMessage,
      });
      await tiktokClient.connect();
      activeClients.tiktok = tiktokClient;
      clients.push(tiktokClient);
    }
  }
...
function getClientsStatus() {
  return {
    ...
    tiktok: activeClients.tiktok ? activeClients.tiktok.isConnected() : false,
  };
}
